############################################
# Locals (naming convention: chrisbarm-*)
############################################
locals {
  name_prefix = var.project_name
  ec2_ami_id  = var.ec2_ami_id != "ami-REPLACE_ME" ? var.ec2_ami_id : data.aws_ssm_parameter.al2023_ami.value
}
...
# Explanation: chrisbarm needs a hyperlane—this VPC is the Millennium Falcon’s flight corridor.
...
# Explanation: chrisbarm wants the private base to call home—EIP gives the NAT a stable “holonet address.”
...
# Explanation: NAT is chrisbarm’s smuggler tunnel—private subnets can reach out without being seen.
...
# Explanation: Private subnets route outbound internet via NAT (chrisbarm-approved stealth).
...
# Explanation: EC2 SG is chrisbarm’s bodyguard—only let in what you mean to.
resource "aws_security_group" "chrisbarm_ec2_sg01" {
  name        = "${local.name_prefix}-ec2-sg01"
  description = "EC2 app security group"
  vpc_id      = aws_vpc.chrisbarm_vpc01.id

  # NOTE: For lab convenience, SSH/HTTP are open to the internet. Tighten to your IP as needed.
  ingress {
    description = "SSH"
    from_port   = 22
    to_port     = 22
...
# Explanation: chrisbarm refuses to carry static keys—this role lets EC2 assume permissions safely.
...
resource "aws_instance" "chrisbarm_ec201" {
...
# Explanation: Metrics are chrisbarm’s growls—when they spike, something is wrong.
