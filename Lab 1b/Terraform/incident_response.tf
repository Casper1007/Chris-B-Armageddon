...
data "aws_cloudwatch_log_group" "lab_rds_app_logs" {
  name = "/aws/ec2/chrisbarm-rds-app"
}
...
resource "aws_cloudwatch_log_metric_filter" "db_error_filter" {
  name           = "DBErrorFilter"
  log_group_name = data.aws_cloudwatch_log_group.lab_rds_app_logs.name
  pattern        = "[ERROR]"
    ...
    Statement = [
      {
        ...
        Resource = "${data.aws_cloudwatch_log_group.lab_rds_app_logs.arn}:*"
      },
      {
        Effect = "Allow"
        Action = [
...
output "log_group_name" {
  ...
  value       = data.aws_cloudwatch_log_group.lab_rds_app_logs.name
}
